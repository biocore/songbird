...
### Installation
First, you'll need to make sure that QIIME 2 version >= 2019.7 is installed before installing
...
10% of your samples.
So if you have 100 samples, you should not have a formula with more than 10 variables.  This measure needs to be used with caution, since the number of categories within each variable will also impact this.  A categorical variable with *k* categories counts as *k-1* variables, so a column with 3 categories will be represented as 2 variables in the model.  Continuous variables will only count as 1 variable.  **Beware of overfitting, though!** You can mitigate the risk of overfitting with the `--differential-prior` parameter.
...
A comprehensive list of the parameters and their default values can be found using 
`qiime songbird multinomial --help` in qiime2 or
`songbird multinomial --help` in the standalone version.

In the example on the Red Sea dataset above, the reason we
